...
To handle game scheduling and notifications, I integrated these through AlarmManager, allowing users to "Set an alarm” for upcoming games. Notifications are managed using Android’s AlarmManager and NotificationManager, giving users occasional alerts prompting them to use te app. WorkManager was considered but was not ideal since its main function is to manage deferrable, long-running background tasks rather than real-time scheduling.

For third-party API integration, the application fetches NFL teams and player data from "api-football.com" NFL API, ensuring users receive up-to-date scores, statistics, and schedules. The data retrieval process is optimised with Retrofit.

To enhance user experience, the application checks for internet connectivity and displays an appropriate message if offline. Lastly, for news content, a WebView loads the latest NFL updates directly from the official NFL website, maintaining an in-app browsing experience without unnecessary redirections.
